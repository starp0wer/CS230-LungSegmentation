...
if __name__ == '__main__':
    ...
    csv_path = '??.csv' #path to csv file of test set
    df = pd.read_csv(csv_path)
    ...
    # Load test data
    im_shape = (256, 256)
    X, y = loadDataGeneral(df_test, im_shape)

    n_test = X.shape[0]
    inp_shape = X[0].shape

    # Load model
    model_name = '??model.hdf5' #replace by the name of your own trained model
    ...
    for xx, yy in test_gen.flow(X, y, batch_size=1):
        ...
        if not os.path.isdir("results/"):
            os.mkdir("results/")
        io.imsave('results/{}'.format(df.iloc[i][0].split('/')[-1]), masked(img, gt, pr, 1))
